...
def main(args):
    ...
    save_dir = './Gender_embeddings_{}'.format(conv.upper())
    ...
    for i, batch in enumerate(test_loader):
        ...
        # Forward pass
        _, embedding, _ = model(network_matrices, net_index)
        np.save(save_dir + f"/embeddings_{i}.npy", embedding.cpu().detach().numpy())
        np.save(save_dir + f"/state_labels_{i}.npy", state_labels.cpu().detach().numpy())
        np.save(save_dir + f"/ind_labels_{i}.npy", ind_labels.cpu().detach().numpy())
